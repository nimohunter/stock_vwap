...
| `npm run verify-data` | Assert every ticker reached the last regular session |
...
**Freshness is verified, not assumed.** Yahoo has repeatedly answered `200` with a full 2-year
payload whose newest bar was a *whole trading session behind* — and because the bar count is
roughly constant on a rolling 2y window, the old script logged an indistinguishable
`Yahoo ✓ (501 bars)` and the nightly Action committed the unchanged bars (plus revised volumes
on older ones) as a green success. The data silently ran a day late. Now every response is
checked against Yahoo's own `meta.regularMarketTime`, which names the last regular session and
so needs no market-holiday calendar:

- a payload short of that session is **retried**, then handed to the Alpha Vantage fallback,
  and its bars are still merged (they carry revisions to earlier days) but reported as `STALE`;
- a bar with a `null` close — a session Yahoo hasn't aggregated yet — is logged instead of
  being dropped in silence;
- the run ends with the newest session reached and an explicit list of any ticker behind it;
- `npm run verify-data` (`--verify`) re-checks the committed files against one reference
  ticker's session in a single request, and `STRICT_FRESHNESS=1` makes a stale refresh exit
  non-zero. Both are off by default for `dev`/`build`, so a bad upstream day warns loudly
  but never breaks the deploy.
...
A second **catch-up run at 11:30 UTC** (07:30 ET, before the open) covers the evening run
coming back a session short — GitHub commonly starts scheduled jobs an hour or two late, and
by the next morning the daily bars are final. A final **Verify data freshness** step runs
*after* the commit, on purpose: whatever data arrived is already saved, and the step is what
turns the run red when the committed set is still behind. A green check now means the data
actually advanced — previously it only meant the script didn't crash.
